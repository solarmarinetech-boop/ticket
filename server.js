...
// ── Gemini processing ──
async function processWithGemini(rawText, username) {
...
try {
  ...
  bot.on('message', async (msg) => {
    ...
    try {
      ...
      const parsed = await processWithGemini(rawText, username);
...
app.post('/api/tickets', async (req, res) => {
  ...
  try {
    const tickets = loadTickets();
    const parsed = await processWithGemini(rawText, username || '@manual');
    const now = new Date();
